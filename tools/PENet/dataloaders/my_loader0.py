...
def plot_to_matrix(img, x, y, depth, points, file_idx):
    ...
    ax.scatter(x, y, s=1, c=depth, linewidths=0, alpha=1, cmap="jet")
    fig_convert.savefig(os.path.join("/opt/data/private/codeN/VirConv/tools/PENet/visual/depth/", file_idx + '.png'), dpi=dpi, bbox_inches='tight', pad_inches=0.0)
    data = cv2.imread(os.path.join("/opt/data/private/codeN/VirConv/tools/PENet/visual/depth/", file_idx + '.png'))
    return data
...
def depth2pointsrgbpm(depth, image, image_ori, calib, lidar, thing_classes, mask_image, classes, scores, boxes2D, lidar_depth, file_idx):
    ...
    calib_path = "/opt/data/private/codeN/VirConv/data/kitti/training/calib/"
    ...
    # # cv2.imwrite(os.path.join("/opt/data/private/codeN/VirConv/tools/PENet/visual/depth/", file_idx + '.jpg') , img_sparse_depth)
    ...
    # lidar index to img
    pts_img, pts_depth = calib.lidar_to_img(lidar_mask[:, :3]) # [:, 2], [:, 1]
    ...
    for i in range(mask_image.shape[0]):
        ...
        if point_type == 'pseudo_points': # pseudo point
            ...
            # one_object_pseudo_point, one_object_point_cloud = torch.from_numpy(one_object_pseudo_point), torch.from_numpy(one_object_point_cloud)
            # if one_object_point_cloud.shape[0] > 0 and one_object_pseudo_point.shape[0] > 0:
            #     _, _, _, pseudo_point_idx = BallQuery(one_object_pseudo_point, one_object_point_cloud, 0.1)
            #     one_object_pseudo_point = one_object_pseudo_point[pseudo_point_idx]
            # one_object_points = torch.cat((one_object_point_cloud, one_object_pseudo_point), 0).numpy()
            ...
            # uni_down_pcd = pcd1.uniform_down_sample(every_k_points=4)
            # cl, ind = uni_down_pcd.remove_radius_outlier(nb_points=30, radius=1.5)
            # one_object_point_cloud = torch.from_numpy(np.array(cl.points))
        ...
        # pcd1 = o3d.geometry.PointCloud()
        # pcd1.points = o3d.utility.Vector3dVector(lidar_object_mask_list[i])
        # o3d.io.write_point_cloud("/opt/data/private/codeN/VirConv/tools/PENet/visual/depth/" + file_idx + "_" + str(i) + "before_lidar.ply", pcd1)
...
def GenerateAnns(one_objecy_class, one_object_points, class_one_object, score_one_object, boxes2D_one_object, calib_path, file_idx, file, forvis):
    if one_object_points.shape[0] != 0:
        ...
        # print(one_objecy_class, one_object_points, class_one_object, score_one_object, boxes2D_one_object)
        ...
        if rects != None:
            for rect in rects:
                ...
                if center_type == 'ori' or center_type == 'prior_linecorner' or center_type == 'prior_center': # original get center of x, y
                ...
                if l1 / l2 > 5 or l2 / l1 > 5: continue
                if l1 >= l2:
                    length, width = l1, l2
                    if center_type == 'ori': length, width = 3.9, 1.6
                    ...
                    if center_type == 'prior_centerline': # prior get center of x, y
                        ...
                        if l1 < 1.5 and l2 < 2:  
                            ...
                            if rotation < 0.7854:
                                ...
                                length, width = 3.9, 1.6
                    length, width = 3.9, 1.6
                print(np.sqrt(center_x ** 2 + center_y ** 2), 111)
                if np.sqrt(center_x ** 2 + center_y ** 2) < 10: continue
