...
class DataProcessor(object):
    ...
    def sample_points(self, data_dict=None, config=None):
        ...
        if num_points < len(points):
            ...
            choice = np.arange(0, len(points), dtype=np.int32)
            if num_points > len(points):
                # print(choice.shape, num_points, len(points), data_dict, 111)
                replace_flag = num_points - len(points) > len(choice)
                extra_choice = np.random.choice(choice, num_points - len(points), replace=replace_flag)
