...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        # print(gt_annos_part)
        # print(gt_annos_part)
        # print([a["bbox"] for a in dt_annos_part])
        if metric == 0:
            gt_boxes = np.concatenate([a["bbox"] for a in gt_annos_part], 0) # kitti
            # gt_boxes = np.concatenate([a["bbox"].reshape(-1, 7) for a in gt_annos_part], 0) # pandaset
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        # dt_datas_list.append(dt_datas.reshape(-1, 9))
        dt_datas_list.append(dt_datas) # kitti
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for j, num_part in enumerate(split_parts):
                    ...
                    # print(dt_datas_list[idx:idx + num_part])
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    overlap_0_25 = np.array([[0.25, 0.25, 0.25, 0.25,
                            ...
                            [0.25, 0.25, 0.25, 0.25,
                             0.25, 0.25, 0.25, 0.25, 0.25, 0.25]])
    min_overlaps = np.stack([overlap_0_7, overlap_0_5, overlap_0_25], axis=0)  # [2, 3, 5]
    class_to_name = { # ['Car', 'Truck', 'Construction_vehicle', 'Bus', 'Trailer', 'Barrier', 'Motorcycle', 'Bicycle', 'Pedestrian', 'Traffic_cone']
        # 0: 'Car',
        # 1: 'Pedestrian',
        # 2: 'Cyclist',
        # 3: 'Van',
